...
/**
 * User: Huangbt
 * Date: 2017/10/18 0018                     //依托于mysqli...的处理 ，
 * Time: 下午 22:10                   //模型类，加载了外部的数据库驱动类和缓存类
 */
...
class BitMod {
    ...
    public function connect() {
        $dbDriver = 'Bit' . ucfirst($this->config['DB_TYPE']);
       require_once( dirname(__FILE__) . '/lib/driver/db/' . $dbDriver . '.php' );
    ...
    public function __call($method, $args) {
        ...
        if (in_array($method, array('field', 'data', 'where', 'group', 'having', 'order', 'limit', 'cache'))) {
            ...
            throw new Exception($method . '方法在BitMod.php类中没有定义');
        }
    }
    ...
    public function query($sql, $params = array(), $is_query = false) {
        ...
        if ($is_query || stripos(trim($sql), 'select') === 0) {
            ...
            $query = $this->db->query($this->sql, $params);     //依托于mysqli...的处理
